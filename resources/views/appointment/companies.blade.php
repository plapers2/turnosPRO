<x-app-layout>
    <main class="flex-1 flex flex-col relative h-full overflow-y-auto bg-surface">
        ...
        <x-header-admin
            ...
            mensaje="Elige el negocio en el que deseas agendar tu cita" />

        <!-- HERO -->
        <div class="relative overflow-hidden bg-gradient-to-br from-primary/10 via-surface to-secondary/10 px-8 py-10 border-b border-outline-variant/20">
            <div class="max-w-2xl">
        ...
        <div class="p-8 pb-20 space-y-12" id="companiesCanvas">
            ...
            <section data-section="{{ $tipo->id }}" class="category-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($tipo->companies as $empresa)
                    <a
                        href="{{ route('appointments.selectServices', $empresa->id) }}"
                        data-name="{{ strtolower($empresa->nombre) }}"
                        class="empresa-card group bg-surface-container-lowest rounded-xl flex flex-col shadow-[0_4px_20px_rgba(95,94,90,0.07)] hover:shadow-[0_8px_30px_rgba(95,94,90,0.13)] transition-all duration-300 hover:-translate-y-1 border border-outline-variant/10 hover:border-primary/30 cursor-pointer overflow-hidden">
                        <!-- Logo / Banner -->
                        <div class="relative w-full h-36 overflow-hidden bg-surface-container flex items-center justify-center">
                            ...
                            <!-- Badge estado -->
                            <span class="absolute top-3 right-3 px-2 py-0.5 rounded-md text-[10px] font-bold font-label
                                            {{ $empresa->estado === 'Activo' ? 'bg-green-500/20 text-green-700' : 'bg-error/20 text-error' }}">
                                {{ $empresa->estado }}
                            </span>
                        </div>
                        ...
                        <div class="p-5 flex flex-col gap-3 flex-1">
                            ...
                            <div class="flex flex-col gap-1.5 mt-auto">
                                <div class="flex items-center gap-2 text-xs text-on-surface-variant">
                                    ...
                                    <span class="line-clamp-1">{{ $empresa->direccion }}</span>
                                </div>
                                <div class="flex items-center gap-2 text-xs text-on-surface-variant">
...
<script>
    ...
    function filterBySearch(query) {
        ...
        sections.forEach(section => {
            const visibleCards = [...section.querySelectorAll('.empresa-card')]
