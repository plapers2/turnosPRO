<x-app-layout>
    <main class="flex-1 flex flex-col relative h-full overflow-y-auto bg-surface">
        ...
        <div
            ...
            <div class="flex items-center gap-5">
                ...
                <div class="flex-1">
                    <p class="text-xs font-semibold tracking-widest uppercase text-primary mb-1 font-label">Paso 2 de 3
                    </p>
        ...
        <div class="p-8 pb-24">
            ...
            <div
                ...
                <div
                    class="w-14 h-14 flex items-center justify-center rounded-full bg-primary/10 text-primary mb-5">
                    <span class="material-symbols-outlined">design_services</span>
                </div>
            ...
            @if(session('error'))
            <div class="mb-5 flex items-start gap-3 px-4 py-3.5 rounded-xl bg-red-50 border border-red-200 text-red-800">
                <span class="material-symbols-outlined text-red-500 text-[20px] mt-0.5 flex-shrink-0">warning</span>
                <div>
                    <p class="text-sm font-semibold">Combinación no disponible</p>
                    <p class="text-xs mt-0.5 text-red-700">{{ session('error') }}</p>
                </div>
            </div>
            @endif
            <!-- Banner de combinación inválida -->
            <div id="invalidBanner" class="hidden mb-5 flex items-start gap-3 px-4 py-3.5 rounded-xl
    bg-red-50 border border-red-200 text-red-800">
                <span class="material-symbols-outlined text-red-500 text-[20px] mt-0.5 flex-shrink-0">warning</span>
                <div>
                    <p class="text-sm font-semibold">Combinación no disponible</p>
                    <p class="text-xs mt-0.5 text-red-700" id="invalidMsg">
                        Ningún profesional puede atender todos los servicios seleccionados en un mismo horario.
                        Intenta con menos servicios o una combinación diferente.
                    </p>
                </div>
            </div>
            <!-- Instrucción -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5" id="servicesGrid">
                ...
                <label data-id="{{ $service->id }}" data-price="{{ $service->price }}"
                    ...
                    <!-- Check indicator -->
                    <div
                        class="check-indicator absolute top-3 right-3 z-10 w-6 h-6 rounded-full border-2
                                border-outline-variant/40 bg-surface flex items-center justify-center transition-all">
                        <span
                            class="material-symbols-outlined text-[14px] text-white hidden check-icon">check</span>
                    </div>
                    ...
                    <div class="p-4 flex flex-col gap-2">
                        <h4
                            class="text-sm font-bold text-on-surface font-headline line-clamp-1 group-hover:text-primary transition-colors">
                            {{ $service->name }}
                        </h4>
                        ...
                        <div class="flex gap-2 mt-1">
                            <span
                                class="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-primary-fixed text-on-primary-fixed text-[11px] font-semibold font-label">
                                <span class="material-symbols-outlined text-[12px]">schedule</span>
                                {{ $service->duration }} min
                            </span>
                            <span
                                class="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-secondary-fixed text-on-secondary-fixed text-[11px] font-semibold font-label">
            ...
            <div id="summaryBar"
                class="fixed bottom-0  md:left-64 left-0 right-0 z-30 bg-surface/95 backdrop-blur-sm border-t border-outline-variant/20
                ...
                <form method="GET" action="{{ route('booking.prepareCreate') }}" id="continueForm" data-company="{{ $company->id }}">
    ...
    bg-primary text-white transition shadow-sm opacity-50 cursor-not-allowed">
                        Continuar
...
<script>
    ...
    const continueBtn = document.querySelector('#continueForm button[type="submit"]');
    const invalidBanner = document.getElementById('invalidBanner');
    const invalidMsg = document.getElementById('invalidMsg');
    const companyId = document.getElementById('continueForm').dataset.company;
    ...
    function updateSummary() {
        ...
        if (checked.length === 1) {
            ...
            continueBtn.classList.remove('opacity-50', 'cursor-not-allowed', 'opacity-75');
            return;
        }

        // 2+ servicios: deshabilitar inmediatamente y validar
        continueBtn.disabled = true;
        continueBtn.classList.add('opacity-50', 'cursor-not-allowed');
        continueBtn.classList.remove('opacity-75');

        const ids = [...checked].map(cb => cb.value);
        clearTimeout(validationTimer);
        setContinueLoading(true);
        validationTimer = setTimeout(() => validarCombinacion(ids), 400);
    }

    async function validarCombinacion(ids) {
        const params = new URLSearchParams({
            company_id: companyId
        });
        ids.forEach(id => params.append('services[]', id));

        try {
            const res = await fetch(`/booking/validar-combinacion?${params}`);
            const data = await res.json();
            setValid(data.valido, data.duracion_total);
        } catch (e) {
            setValid(true);
        } finally {
    ...
    function setValid(valido, duracionTotal = null) {
        continueBtn.disabled = !valido;
        continueBtn.classList.toggle('opacity-50', !valido);
        continueBtn.classList.toggle('cursor-not-allowed', !valido);
        ...
        if (valido) {
            ...
            if (duracionTotal) {
                invalidMsg.textContent =
                    `La combinación requiere ${duracionTotal} min en total y ningún profesional ` +
                    `disponible puede cubrir todos los servicios en ese bloque. ` +
                    `Intenta con menos servicios o una combinación diferente.`;
            }
        }
    }
    ...
    function setContinueLoading(loading) {
        if (loading) {
            continueBtn.disabled = true;
            continueBtn.classList.add('opacity-75');
            continueBtn.classList.remove('opacity-50', 'cursor-not-allowed');
        }
        continueBtn.disabled = loading;
        continueBtn.classList.toggle('opacity-75', loading);
    }
