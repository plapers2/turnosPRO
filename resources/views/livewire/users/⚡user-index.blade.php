    ...
    <div class="px-8 pb-20">
        <div
            ...
            <div wire:loading.class="opacity-50 pointer-events-none transition-opacity duration-200"
                wire:target="search, status, role" class="hidden md:block overflow-x-auto">
                        ...
                        @forelse ($users as $user)
                            <tr class="hover:bg-surface/40 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-xs text-on-surface-variant">{{ $user->phone ?? 'Sin teléfono' }}</p>
                                </td>
                                    ...
                                    @if ($user->trashed())
                                        <button wire:click="restoreUser({{ $user->id }})"
                                            ...
                                            <span wire:loading.remove
                                                wire:target="restoreUser({{ $user->id }})">Restaurar</span>
                                    ...
                                    @else
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('users.show', $user->id) }}"
                                                class="text-blue-700 hover:text-blue-800 transition">Ver</a>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-16">
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <p class="text-on-surface-variant">No se encontraron usuarios</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            {{-- MOBILE --}}
            <div wire:loading.class="opacity-50 pointer-events-none transition-opacity duration-200"
                wire:target="search, status, role" class="md:hidden space-y-4 p-4">
                @forelse ($users as $user)
                    <div class="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-4 shadow-sm">
                        ...
                        <div class="flex justify-end gap-4 mt-4 border-t pt-3">
                            @if ($user->trashed())
                                <button wire:click="restoreUser({{ $user->id }})"
                                    class="text-green-600 hover:text-green-800 transition">Restaurar</button>
                            @else
                                <a href="{{ route('users.show', $user->id) }}"
                                    class="text-blue-700 hover:text-blue-800 transition">Ver</a>
                                <a href="{{ route('users.edit', $user->id) }}"
                ...
                @empty
                    <p class="text-center text-sm text-gray-400">No hay usuarios</p>
                @endforelse
            </div>
