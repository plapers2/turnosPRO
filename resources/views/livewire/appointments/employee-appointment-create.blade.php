<div>
    ...
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
        ...
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-green-600 text-white text-sm font-medium px-5 py-3.5 rounded-2xl shadow-xl max-w-sm w-full mx-4">
    ...
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 6000)"
        ...
        x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-end="opacity-0 translate-y-4"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-red-600 text-white text-sm font-medium px-5 py-3.5 rounded-2xl shadow-xl max-w-sm w-full mx-4">
    ...
    <div class="p-6 pb-24 max-w-7xl mx-auto">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                @if($companyData)
                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/20 shadow-sm px-5 py-3.5 flex items-center gap-4">
                    <div class="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-primary/10 flex items-center justify-center border border-outline-variant/20">
                        @if($companyData['logo'])
                        <img src="{{ $companyData['logo'] }}" alt="{{ $companyData['name'] }}" class="w-full h-full object-cover">
                        @else
                        <span class="text-xs font-bold text-primary/50">{{ strtoupper(substr($companyData['name'], 0, 2)) }}</span>
                        @endif
                    </div>
                    <div class="flex-1">
                    ...
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-primary/10 text-primary text-[11px] font-semibold">
                ...
                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/20 shadow-sm">
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-surface-container border border-outline-variant/20">
                            <div class="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                                <span class="text-sm font-bold text-primary/60">{{ strtoupper(substr($clienteData['name'], 0, 1)) }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-on-surface-variant">{{ $clienteData['email'] }}
                                    @if($clienteData['phone']) · {{ $clienteData['phone'] }} @endif
                                </p>
                            </div>
                        ...
                        @else
                        {{-- Buscador --}}
                        <div class="relative" x-data="{ open: false }" x-on:mousedown.outside="open = false; $wire.call('cerrarBuscador')">
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant/50 text-[18px]">search</span>
                                <input
                                    wire:model.live.debounce.300ms="clienteSearch"
                                    wire:focus="cargarTodosClientes"
                                ...
                                <div wire:loading wire:target="updatedClienteSearch" class="absolute right-3 top-1/2 -translate-y-1/2">
                            ...
                            <div class="absolute z-20 w-full mt-1 bg-surface-container-lowest border border-outline-variant/20 rounded-xl shadow-lg overflow-hidden max-h-72 overflow-y-auto">
                                @foreach($clienteResultados as $resultado)
                                <button
                                    type="button"
                                    data-user-id="{{ $resultado['user_id'] }}"
                                    ...
                                    <div class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                                        <span class="text-xs font-bold text-primary/60">{{ strtoupper(substr($resultado['name'], 0, 1)) }}</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-on-surface">{{ $resultado['name'] }}</p>
                                        <p class="text-xs text-on-surface-variant truncate">{{ $resultado['email'] }}
                                            @if($resultado['phone']) · {{ $resultado['phone'] }} @endif
                                        </p>
                                    </div>
                                </button>
                                @endforeach
                            ...
                            <div class="absolute z-20 w-full mt-1 bg-surface-container-lowest border border-outline-variant/20 rounded-xl shadow-lg px-4 py-3">
                                <p class="text-xs text-on-surface-variant text-center">Sin resultados para "{{ $clienteSearch }}"</p>
                            </div>
                            @endif
                        </div>
                        <p class="text-xs text-on-surface-variant mt-2">Solo aparecen clientes registrados en esta empresa.</p>
                        @endif
                    </div>
                </div>
                            ...
                            <button wire:click="toggleServicio({{ $service['id'] }})"
                                ...
                                <div class="w-full h-24 overflow-hidden bg-surface-container">
                                    ...
                                    <div class="w-full h-full bg-gradient-to-br from-primary/10 to-secondary/5 flex items-center justify-center">
                                        <span class="material-symbols-outlined text-primary/30" style="font-size:32px">spa</span>
                                    </div>
                                    @endif
                                </div>
                ...
                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/20 shadow-sm overflow-hidden" id="emp-datepicker-card">
                    <div class="px-6 py-4 border-b border-outline-variant/20 flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-bold text-on-surface font-headline">
                    ...
                    <div wire:ignore>
                        ...
                        <div id="emp-dp-slots-wrapper" class="hidden px-5 pb-5 border-t border-outline-variant/20 pt-4">
                            ...
                            <div id="emp-dp-slots-empty" class="hidden text-xs text-on-surface-variant text-center py-4">
                    ...
                    <div id="emp-dp-data"
                        ...
                        data-empleado="{{ $esEmpleado ? $empleadoUserId : '' }}"
                        class="hidden">
                    </div>
                </div>
                @endif
                ...
                @if($fecha && $hora)
                <div data-paso-emp="4" class="bg-surface-container-lowest rounded-xl border border-outline-variant/20 shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 space-y-6" wire:loading.class="opacity-60 pointer-events-none" wire:target="seleccionarSlot">
                        ...
                        <div class="flex flex-col items-center justify-center py-8 text-center">
                            ...
                            <p class="text-xs text-on-surface-variant">No hay profesionales disponibles en este horario.</p>
                        </div>
                        @else
                        ...
                        <div>
                            <div class="flex items-center gap-3 mb-3">
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-on-surface-variant">
                                        {{ $grupo['hora_inicio'] }} → {{ $grupo['hora_fin'] }} · {{ $grupo['service']['duration'] }} min
                                    </p>
                                </div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                <button wire:click="seleccionarProfesional({{ $serviceId }}, {{ $prof['id'] }})"
                                    ...
                                    <span class="text-sm font-semibold {{ $isSelected ? 'text-primary' : 'text-on-surface' }}">{{ $prof['name'] }}</span>
                                    @if($isSelected)
                ...
                @php
                $todosAsignados = count($selectedProfesionales) === count($selectedServices) && count($selectedServices) > 0;
                @endphp
            ...
            <div class="space-y-4">
                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/20 shadow-sm sticky top-4">
                    ...
                    <div class="p-4 space-y-3">
                        @if(!$clienteData)
                        <div class="flex flex-col items-center justify-center py-8 text-center">
                            <span class="material-symbols-outlined text-on-surface-variant/30 text-4xl mb-2">person_search</span>
                        ...
                        <div class="bg-surface-container rounded-xl p-3.5 border border-outline-variant/10">
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <p class="text-xs font-bold text-on-surface line-clamp-1">{{ $cita['servicios'] }}</p>
            ...
            function renderMonth(gridEl, year, month) {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    if (isPast) {
                        ...
                        btnClass += key === selectedDate ? 'bg-primary text-white shadow-md scale-105' : 'hover:bg-primary/10 hover:text-primary text-on-surface cursor-pointer';
                        dotHtml = `<span class="w-1.5 h-1.5 rounded-full mt-0.5 ${key === selectedDate ? 'bg-white' : 'bg-emerald-400'}"></span>`;
                    } else if (state === 'low') {
                        btnClass += key === selectedDate ? 'bg-primary text-white shadow-md scale-105' : 'hover:bg-orange-50 hover:text-orange-700 text-on-surface cursor-pointer';
                        dotHtml = `<span class="w-1.5 h-1.5 rounded-full mt-0.5 ${key === selectedDate ? 'bg-white' : 'bg-orange-400'}"></span>`;
                    } else {
