...
@if ($paginator->hasPages())
    <div class="border-t border-outline-variant/30 px-2 pt-4 pb-1">
        <div class="flex items-center justify-between gap-2 text-sm">
            ...
            {{-- Anterior --}}
            <button wire:click="previousPage" wire:loading.attr="disabled" @disabled(!$paginator->onFirstPage())
                class="px-3 py-1.5 rounded-lg border border-outline-variant/30
            ...
            {{-- Siguiente --}}
            <button wire:click="nextPage" wire:loading.attr="disabled" @disabled($paginator->onLastPage())
                class="px-3 py-1.5 rounded-lg border border-outline-variant/30
                       bg-surface-container text-on-surface text-xs font-medium
                       disabled:opacity-40 disabled:cursor-not-allowed
                       hover:bg-surface-container-high transition-colors">
