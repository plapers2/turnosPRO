...
<header
    class="relative mx-0 mb-5 overflow-hidden rounded-2xl border border-outline-variant/30
           bg-surface-container-lowest px-4 py-5 sm:px-6 sm:py-7
    ...
    {{-- Toggle de vista --}}
    <div class="w-full flex flex-wrap items-center gap-1.5 rounded-xl border border-outline-variant/40
                bg-surface-container p-1 shrink-0">

        <button wire:click="$set('view', 'list')" @class([
            'flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12.5px] font-semibold transition-all duration-150',
            'bg-surface-container-lowest text-primary shadow-sm border border-outline-variant/30' => $view === 'list',
            'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface border border-transparent' => $view !== 'list',
        ]) title="Vista lista">
        ...
        <button wire:click="$set('view', 'calendar')" @class([
            'flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12.5px] font-semibold transition-all duration-150',
            'bg-surface-container-lowest text-primary shadow-sm border border-outline-variant/30' => $view === 'calendar',
            'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface border border-transparent' => $view !== 'calendar',
        ...
        <button wire:click="$set('view', 'availability')" @class([
            'inline-flex items-center gap-2 px-3 py-2 text-[12.5px] font-semibold rounded-lg transition-colors',
            'bg-surface-container-lowest text-primary shadow-sm border border-outline-variant/30' => $view === 'availability',
            'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface border border-transparent' => $view !== 'availability',
        ])>
        ...
        {{-- Spacer para empujar los últimos botones a la derecha en pantallas grandes --}}
        ...
        <a href="{{ route('employee.appointment.create') }}"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-white text-[12.5px] font-semibold hover:bg-primary/90 transition">
